...
function getSlideWidth() {
  ...
  function initParallaxAbout() {
    const aboutWrapper = document.getElementById('about-wrapper');
    const bg1 = document.querySelector('.about-bg-1');
    const bg2 = document.querySelector('.about-bg-2');
    
    if (!aboutWrapper || !bg1 || !bg2) return;
    ...
    function updateParallax() {
      ...
      // Начинаем когда секция входит во viewport
      if (wrapperTop <= 0 && wrapperTop > -wrapperHeight + windowHeight) {
        const scrollProgress = Math.abs(wrapperTop) / (wrapperHeight - windowHeight);
        
        if (scrollProgress < 0.33) {
          // Первый экран - первый фон полностью виден
          bg1.style.opacity = '1';
          bg2.style.opacity = '0';
        } else if (scrollProgress >= 0.33 && scrollProgress < 0.66) {
          // Второй экран - плавный переход между фонами
          const transitionProgress = (scrollProgress - 0.33) / 0.33;
          bg1.style.opacity = String(1 - transitionProgress);
          bg2.style.opacity = String(transitionProgress);
        } else {
          // Третий экран - второй фон полностью виден
          bg1.style.opacity = '0';
          bg2.style.opacity = '1';
        }
      }
  ...
  window.addEventListener('resize', () => {
    // Удаляем старые обработчики и создаем новые при изменении размера окна
    initParallaxAbout();
  });
